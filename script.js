...
function updateChrome() {
  ...
  statusLineEl.textContent = unlockedRestricted
    ? `Status: live reconstruction, ${unlockedRestricted} restricted node(s) available`
    : "Status: live reconstruction";
...
function renderHub() {
  ...
  const traceState = state.phraseSources.length >= 3 ? "phrase cluster correlated" : "phrase cluster incomplete";

  return `
    <p class="lede">The Blackwood House file is built around sequence drift, conflicting testimony, repeated language, and restored traces. This hub lets those parts accumulate into a single reconstruction workspace.</p>
    <div class="hub-grid">
      <section class="hub-card">
        <h3>Witness Graph</h3>
        <p>Five testimony nodes with incompatible timings, motives, and omissions. Opening one node changes how the others read.</p>
        <p><span class="metric">${interviewsRead}/5</span></p>
        <p><span class="tag">sequence drift tracked live</span></p>
      </section>
      <section class="hub-card">
        <h3>Evidence Mesh</h3>
        <p>Administrative records, sensor output, donor drafts, and device traces. The point is not to read them once; it is to compare them against each other.</p>
        <p><span class="metric">${evidenceRead}</span></p>
        <p><span class="tag">nodes ingested this run</span></p>
      </section>
      <section class="hub-card">
        <h3>Conflict Engine</h3>
        <p>The timeline is generated from what you have opened, so confidence changes as new records surface. Static chronology would be misleading here.</p>
        <p><span class="metric">${getContradictionCount()}</span></p>
        <p><span class="tag">active contradiction markers</span></p>
      </section>
      <section class="hub-card">
        <h3>Recovery Layer</h3>
        <p>Hidden nodes restore only after behavioral triggers are met. The interface rewards correlation, not passive reading.</p>
        <p><span class="metric">${restrictedRead}</span></p>
        <p><span class="tag">${traceState}</span></p>
      </section>
    </div>
    <p>Follow the recurring phrase when it appears, open redactions that look synthetically clean, and revisit earlier nodes after later ones expose their bias. The system keeps a session-level reconstruction state and recalculates what the archive will show you.</p>
  `;
}
...
function renderTimeline() {
  ...
  return `
    <p class="lede">This timeline recalculates as testimony, sensor logs, and recovered drafts enter the graph. Confidence changes with the record instead of pretending the sequence was ever settled.</p>
    ${entries.map(entry => `
...
function renderNotes() {
  const notes = [
    "The opening contradiction is temporal, not emotional. Everyone agrees Gideon died after midnight. What fractures is the interval in which he was still alive and therefore still capable of arranging the story. That is why this interface tracks sequence drift instead of presenting a clean chapter order.",
    "Mara's interview contains the narrowest denial in the archive: she says she did not serve Gideon anything. The kitchen note later says someone in the blue room already brought his whiskey through. This does not make her the killer. It does make her one of the actors who changed system state before the death occurred.",
    "Repeated phrase: 'for the record.' Not just verbal tic. In Gideon's mouth it appears to mean: what follows will be arranged for future leverage. In others it sometimes means the opposite: a desperate attempt to place one true sentence somewhere durable before it is revised. On this site, tracing the phrase is an unlock mechanic because the phrase is itself evidence."
  ];
  ...
  notes.push("Recurring sentence from multiple margins: the record is not the event. This has moved from warning to method. The archive itself gates nodes, recalculates sequence, and changes interpretation based on what has already surfaced.");

  return `
    <p class="lede">These notes function like a live analyst layer. They update when new records change the model and keep the investigation focused on what the archive is actually doing.</p>
...
function renderAccusation() {
  ...
  return `
    <p class="lede">This page is the synthesis console. Instead of turning pages toward a predetermined ending, you submit a theory assembled from the evidence graph and the system classifies the interpretation your investigation supports.</p>
